...
public class ATGV4 {
    ...
    private static void getRaceDetails(String raceName, int expectedRaceCount) {
        ...
        By raceHeader = By.xpath("//button[@data-test-id='race-menu-leg']");
        elementWait(raceHeader); // wait for the element
        List<WebElement> races = driver.findElements(raceHeader);
        //check the count
        int raceCount = races.size();
        Assert.assertEquals(raceCount, expectedRaceCount);
    }
    ...
    public static void selectHorse(List<String> myHorsesSelectionPerRace) {
        for (String selection : myHorsesSelectionPerRace) {
            ...
            if (selection.equalsIgnoreCase("Alla")) {
                ...
                // Get the unscratched horse number and save it in unScratchedHorses
